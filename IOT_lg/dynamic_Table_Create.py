...
def create_table(table_name):
    create_query = f"""
        CREATE TABLE iot_lg.{table_name} (
...
def create_table_all_table_name(table_name):
    ...
    try:
        cur.execute(create_query)
        db_connection.commit()
        logging.info(f"Table created in iot_lg schema : {table_name}.")
...
def insert_all_tables_names(table_name):
    query = f"""
     INSERT INTO iot_lg.{table_name} (device_type, model_name)
    SELECT DISTINCT device_type, model_name FROM iot_lg.device_staging
...
if not unique_devices:
    # Query to get all table names in the 'iot_lg' schema
    cur.execute("""
        SELECT table_name 
        FROM information_schema.tables 
        WHERE table_schema = 'iot_lg'
    ...
    for table_name in existing_tables:  # assuming you have the list of all table names already
        ...
        cur.execute(f"""
               ...
               WHERE NOT EXISTS (
                   SELECT 1
                   FROM iot_lg.device_staging
                   WHERE device_staging.device_id = iot_lg.{table_name}.device_id
    ...
    for device_type, model_name in unique_devices:
        ...
        sql = f"""
            WITH upserted AS (
                INSERT INTO iot_lg.{table_name} (device_id, device_type, model_name, alias, reportable, updated_at, log_action)
                ...
                SET alias = EXCLUDED.alias, 
                    ...
                    updated_at = NOW(),
                    log_action = 'update'
                    WHERE iot_lg.{table_name}.alias IS DISTINCT FROM EXCLUDED.alias
                    OR iot_lg.{table_name}.reportable IS DISTINCT FROM EXCLUDED.reportable
...
# cur.execute("TRUNCATE TABLE iot_lg.device_staging;")
# db_connection.commit()
